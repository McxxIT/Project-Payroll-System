document.addEventListener("DOMContentLoaded", function () {
    ...
    const tableBody = document.querySelector(".table-body-2");
    const rows = tableBody.querySelectorAll(".table-row-2");
    ...
    searchInput.addEventListener("input", function (e) {
        ...
        rows.forEach(row => {
            const cells = row.querySelectorAll(".payslip-cell"); // Assuming all cells have this class

            // Skip processing if this row has no cells or if it's the header row
            if (cells.length === 0 || row.classList.contains("table-header-2")) {
                return;
            }
